...
build_treatment_staging <- function(prepared_treatment, first_treatmentid) {
  if (anyNA(prepared_treatment$source_treatmentid) ||
      anyDuplicated(prepared_treatment$source_treatmentid)) {
    stop("Each prepared treatment ID must identify exactly one event.", call. = FALSE)
  }
...
build_area_treatment_staging <- function(prepared_links, prepared_area, area_crosswalk,
                                         treatment_crosswalk, database, projectid) {
  child_keys <- prepared_area$area_key[!is.na(prepared_area$parent_area_key)]
  if (anyNA(prepared_links$area_key) ||
      !all(prepared_links$area_key %in% child_keys)) {
    stop("Only child areas may receive treatment links.", call. = FALSE)
  }
  if (anyDuplicated(area_crosswalk$area_key) ||
      anyDuplicated(treatment_crosswalk$source_treatmentid)) {
    stop("Area or treatment crosswalk contains duplicate lookup keys.", call. = FALSE)
  }
  result <- prepared_links |>
    dplyr::left_join(area_crosswalk, by = "area_key", relationship = "many-to-one") |>
