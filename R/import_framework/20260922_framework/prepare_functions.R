...
### One harmonized treatment ID represents one event, which may have several
### action/detail rows. Conflicting event timing or context is flagged for
### preprocessing review; detail rows are never silently split into events.
...
audit_treatment_event_identity <- function(treatments) {
  assert_columns(
    treatments,
    c("treatmentid", "DB", "siteid", "restorationtype", "tsr_start_year",
      "trt_year", "trt_tsr", "treatmentmonth", "treatmentday"),
    "treatments"
  )
  treatments |>
    dplyr::mutate(
    ...
    dplyr::group_by(.data$source_treatmentid) |>
    dplyr::summarise(
      treatment_rows = dplyr::n(),
      database_values = dplyr::n_distinct(prepare_na_if_blank(.data$DB), na.rm = TRUE),
      site_values = dplyr::n_distinct(prepare_na_if_blank(.data$siteid), na.rm = TRUE),
      restoration_type_values = dplyr::n_distinct(
        prepare_na_if_blank(.data$restorationtype), na.rm = TRUE
      ),
      restoration_start_values = dplyr::n_distinct(
        prepare_na_if_blank(.data$tsr_start_year), na.rm = TRUE
      ),
      year_values = dplyr::n_distinct(prepare_na_if_blank(.data$trt_year), na.rm = TRUE),
      month_values = dplyr::n_distinct(prepare_na_if_blank(.data$treatmentmonth), na.rm = TRUE),
      day_values = dplyr::n_distinct(prepare_na_if_blank(.data$treatmentday), na.rm = TRUE),
      weeks_since_restoration_values = dplyr::n_distinct(
        prepare_na_if_blank(.data$trt_tsr), na.rm = TRUE
      ),
      .groups = "drop"
    ) |>
    dplyr::mutate(
      review_status = dplyr::case_when(
        is.na(.data$source_treatmentid) ~ "missing treatment ID",
        dplyr::if_any(dplyr::ends_with("_values"), ~ .x > 1L) ~
          "conflicting context or timing within event",
        TRUE ~ "consistent event"
      )
    ) |>
    dplyr::arrange(.data$review_status, .data$source_treatmentid)
}

assert_treatment_event_consistency <- function(treatments) {
  audit <- audit_treatment_event_identity(treatments)
  issues <- audit |>
    dplyr::filter(.data$review_status != "consistent event")
  if (nrow(treatments) == 0L || nrow(issues) > 0L) {
    stop(
      "Treatment-event IDs have missing keys or conflicting context/timing. ",
      "Review in preprocessing before prepare; examples: ",
      paste(utils::head(issues$source_treatmentid, 5L), collapse = ", "),
      call. = FALSE
    )
  }
  invisible(audit)
...
prepare_treatment_event_rows <- function(treatments) {
  assert_columns(
    treatments,
    c("treatmentid", "restorationtype", "treatment_category", "treatment_type"),
    "treatments"
  )
  assert_treatment_event_consistency(treatments)
...
first_event_value <- function(x) {
  present <- which(!is.na(x))
  if (length(present) == 0L) return(x[NA_integer_][[1]])
  x[[present[[1]]]]
}

collapse_event_text <- function(x) {
  values <- unique(prepare_na_if_blank_or_token(x))
  values <- values[!is.na(values)]
  if (length(values) == 0L) NA_character_ else paste(values, collapse = "; ")
}

event_detail_label <- function(x) {
  values <- unique(x[!is.na(x)])
  if (length(values) == 0L) NA_character_ else if (length(values) == 1L) values[[1]] else "multiple"
}
...
prepare_treatments <- function(treatments, route_other_treatment_to_notes = FALSE) {
  assert_columns(
    ...
    c(
      ...
      "treatment_category", "treatment_type"
    ),
    "treatments"
  )
  ...
  prepared <- prepare_treatment_event_rows(treatments) |>
    ...
    dplyr::group_by(.data$source_treatmentid) |>
    dplyr::summarise(
      database = first_event_value(.data$database),
      source_treatment_event_key = first_event_value(.data$source_treatment_event_key),
      event_category = event_detail_label(.data$event_category),
      event_type = event_detail_label(.data$event_type),
      siteid = first_event_value(.data$siteid),
      restoration_type = first_event_value(.data$restoration_type),
      restoration_start_year = first_event_value(.data$restoration_start_year),
      year = first_event_value(.data$year),
      month = first_event_value(.data$month),
      day = first_event_value(.data$day),
      weeks_since_restoration = first_event_value(.data$weeks_since_restoration),
      other_treatment_source = collapse_event_text(.data$other_treatment_source),
      other_treatment = collapse_event_text(.data$other_treatment),
      notes = collapse_event_text(.data$notes),
      .groups = "drop"
    )
  ...
  if (anyDuplicated(prepared$source_treatmentid) ||
      nrow(prepared) != dplyr::n_distinct(prepare_na_if_blank(treatments$treatmentid))) {
    stop("Prepared treatment events did not resolve one-to-one with treatment IDs.",
         call. = FALSE)
  }
...
prepare_area_treatment_links <- function(vegresults, prepared_treatments, areas) {
  ...
  if (anyNA(observed$source_treatmentid) || anyNA(observed$block) ||
      anyNA(observed$replicate)) {
    stop("Every treated observation needs a treatment ID, block, and child-area ",
         "replicate. Review block-only observations before prepare.", call. = FALSE)
  }
  ...
  child_keys <- areas$area_key[!is.na(areas$parent_area_key)]
  if (!all(links$area_key %in% child_keys)) {
    stop("Treatment events may link only to child areas, not parent blocks.",
         call. = FALSE)
  }
