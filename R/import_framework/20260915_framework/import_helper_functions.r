...
upload_to_supabase <- function(
  ...
  # Encode each object-key segment without encoding the path separators.
  # This is required for source file names containing spaces, %, or other
  # URL-reserved characters.
  encoded_destination_path <- paste(
    vapply(
      strsplit(gsub("\\\\", "/", destination_path), "/", fixed = TRUE)[[1]],
      curl::curl_escape,
      character(1)
    ),
    collapse = "/"
