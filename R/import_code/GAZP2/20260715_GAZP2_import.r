### This is to import GAZP2 into the SQL database from the Excel format
### It will be a second draft of the larger import process
...
# Names of all sheets
sheets <- getSheetNames("data/harmonized/GAZP/GAZP2/GAZP2.xlsx")

# Individual sheets
study <- read.xlsx("data/harmonized/GAZP/GAZP2/GAZP2.xlsx", sheet = "study")
site <- read.xlsx("data/harmonized/GAZP/GAZP2/GAZP2.xlsx", sheet = "site")
treatments <- read.xlsx("data/harmonized/GAZP/GAZP2/GAZP2.xlsx", sheet = "treatments")
timepoints <- read.xlsx("data/harmonized/GAZP/GAZP2/GAZP2.xlsx", sheet = "timepoints")
trtrates <- read.xlsx("data/harmonized/GAZP/GAZP2/GAZP2.xlsx", sheet = "trtrates")
refs <- read.xlsx("data/harmonized/GAZP/GAZP2/GAZP2.xlsx", sheet = "refs")
vegresults <- read.xlsx("data/harmonized/GAZP/GAZP2/GAZP2.xlsx", sheet = "vegresults")
...
stg_project <- study |>
  transmute(
    ...
    date_received = as.Date("2018-01-15") ############# CHANGE PER PROJECT
  ) |>
  distinct()
...
stg_project_data_accessibility <- study |>
  ...
  mutate(
    date_received = as.Date("2018-01-15") # CHANGE PER PROJECT and update when refs sheet has a value
  ) |>
  transmute(
    database,
    projectid,
...
# For GAZP2: The year/month/day columns do not reflect the timing of the treatment
# They are the date of seeding, where many of the treatments are irrigation post-seeding
# Correct here
stg_treatment <- stg_treatment |>
  mutate(
    original_date = lubridate::make_date(
      year = as.integer(year),
      month = as.integer(month),
      day = as.integer(day)
    ),
    corrected_date = original_date +
      lubridate::weeks(weeks_since_restoration),
    year = if_else(
      !is.na(corrected_date),
      as.numeric(lubridate::year(corrected_date)),
      year
    ),
    month = if_else(
      !is.na(corrected_date),
      as.numeric(lubridate::month(corrected_date)),
      month
    ),
    day = if_else(
      !is.na(corrected_date),
      as.numeric(lubridate::day(corrected_date)),
      day
    )
  ) |>
  select(-original_date, -corrected_date)
...
stg_treatment |>
  ...
  arrange(weeks_since_restoration, treatmentid)
...
crosswalk_dir <- "crosswalk_tables/GAZP/GAZP2"
dir.create(crosswalk_dir, recursive = TRUE, showWarnings = FALSE)

write_csv(
  harmonized_SQL_crosswalk,
  file.path(crosswalk_dir, "GAZP2_harmonized-SQL_crosswalk.csv")
)
...
# Crosswalk table
upload_to_supabase(
  local_file = "crosswalk_tables/GAZP/GAZP2/GAZP2_harmonized-SQL_crosswalk.csv",
  destination_path = "GAZP/GAZP2/crosswalks/GAZP2_harmonized-SQL_crosswalk.csv",
...
upload_to_supabase(
  local_file =
    "data/harmonized/GAZP/GAZP2/GAZP2.xlsx",

  destination_path =
    "GAZP/GAZP2/harmonized/GAZP2.xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP2/Fire complex description.docx",

  destination_path =
    "GAZP/GAZP2/source/Fire_complex_description.docx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP2/James fire seeding data.xlsx",

  destination_path =
    "GAZP/GAZP2/source/James_fire_seeding_data.xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP2/Meta-data_Bartlett.xlsx",

  destination_path =
    "GAZP/GAZP2/source/Meta-data_Bartlett.xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP2/Meta-data_Butte.xlsx",

  destination_path =
    "GAZP/GAZP2/source/Meta-data_Butte.xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP2/Meta-data_Egley.xlsx",

  destination_path =
    "GAZP/GAZP2/source/Meta-data_Egley.xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP2/Meta-data_Roundtop.xlsx",

  destination_path =
    "GAZP/GAZP2/source/Meta-data_Roundtop.xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP2/updated  Meta-data_Butte (2).xlsx",

  destination_path =
    "GAZP/GAZP2/source/updated_Meta-data_Butte_(2).xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP2/Updated Meta-data_Bartlett.xlsx",

  destination_path =
    "GAZP/GAZP2/source/Updated_Meta-data_Bartlett.xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP2/Updated Meta-data_Egley.xlsx",

  destination_path =
    "GAZP/GAZP2/source/Updated_Meta-data_Egley.xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP2/Updated Meta-data_Roundtop.xlsx",

  destination_path =
    "GAZP/GAZP2/source/Updated_Meta-data_Roundtop.xlsx",
...
upload_to_supabase(
  local_file =
    "R/import_code/GAZP2/20260715_GAZP2_import.r",

  destination_path =
    "GAZP/GAZP2/code/20260715_GAZP2_import.r",
