### This is to import GAZP1 into the SQL database from the Excel format
### It will be a first draft of the larger import process
...
# Names of all sheets
sheets <- getSheetNames("data/harmonized/GAZP/GAZP1/GAZP1.xlsx")

# Individual sheets
study <- read.xlsx("data/harmonized/GAZP/GAZP1/GAZP1.xlsx", sheet = "study")
site <- read.xlsx("data/harmonized/GAZP/GAZP1/GAZP1.xlsx", sheet = "site")
treatments <- read.xlsx("data/harmonized/GAZP/GAZP1/GAZP1.xlsx", sheet = "treatments")
timepoints <- read.xlsx("data/harmonized/GAZP/GAZP1/GAZP1.xlsx", sheet = "timepoints")
trtrates <- read.xlsx("data/harmonized/GAZP/GAZP1/GAZP1.xlsx", sheet = "trtrates")
refs <- read.xlsx("data/harmonized/GAZP/GAZP1/GAZP1.xlsx", sheet = "refs")
vegresults <- read.xlsx("data/harmonized/GAZP/GAZP1/GAZP1.xlsx", sheet = "vegresults")
...
stg_project <- study |>
  transmute(
    ...
    date_received = as.Date("2018-03-01") ############# CHANGE PER PROJECT
  ) |>
  distinct()
...
stg_project_data_accessibility <- study |>
  ...
  mutate(
    date_received = as.Date("2018-03-01") # CHANGE PER PROJECT and update when refs sheet has a value
  ) |>
  transmute(
    database,
    projectid,
...
if (nrow(new_locations) > 0) {
  new_locations <- new_locations |>
    mutate(
      locationid = next_ids(con, table = "location", id_col = "locationid", n = n())
    )
}
...
# Change Australia as continent to Oceania for consistency with UN region names
...
## Quick fix for GAZP1 only: changing to Oceania broke the join
stg_project_location$locationid <- 1
...
stg_site <- site_matched |>
  mutate(
    siteid = coalesce(existing_siteid, source_siteid)
  ) |>
  transmute(
    siteid,
    name,
...
# ---- paper ----
# refs is empty for GAZP1, but this keeps the staged object explicit.
...
#### Group 2: Site attributes
# fix this to not pull in a species lookup, but rely on the one created at the start for the invasive table
# ---- lookup tables ----

lu_classification <- get_lookup_table(con, "classification")
lu_disturbance <- get_lookup_table(con, "disturbance")
lu_species_names <- get_lookup_table(con, "species_names")
...
group2_internal_issues <- purrr::imap_dfr(
  group2_staged,
...
# For GAZP1: plot = treatmentid + block + replicate.
...
# Correct for missing size and units from study worksheet

stg_area <- stg_area |>
  mutate(size = ifelse(type == "plot", 8, size),
         units = ifelse(type == "plot", "m2", units))
...
# GAZP1 ONLY: Grading should not be at tsr = 1 treatments; this is a correction before SQL upload
stg_treatment <- stg_treatment |>
  mutate(
    grading = if_else(
      treatmentid %in% treatment_crosswalk$treatmentid[treatment_crosswalk$source_trt_tsr == 1],
      NA_character_,
      grading
    )
  )
...
group3_internal_issues <- purrr::imap_dfr(
  group3_staged,
  function(tbl, tbl_name) {
...
# Correction for GAZP1
stg_treatment_irrigation$notes <- "evenly applied during four events over seven days"
...
group4_internal_issues <- purrr::imap_dfr(
  group4_staged,
  function(tbl, tbl_name) {
    if (nrow(tbl) == 0) return(NULL)
...
group5_internal_issues <- purrr::imap_dfr(
  group5_staged,
  function(tbl, tbl_name) {
    if (nrow(tbl) == 0) return(NULL)
...
# Correct for GAZP1: response * 8 to return it to the original counts within the 8m2 plots
stg_veg_result <- stg_veg_result |>
  mutate(response = response * 8)

# ---- validation ----
veg_result_internal_issues <- validate_staged_table(
  stg_tbl = stg_veg_result,
  target_table = "veg_result",
  constraints_tbl = import_registry$constraints
)

veg_result_internal_issues
...
crosswalk_dir <- "crosswalk_tables/GAZP/GAZP1"
dir.create(crosswalk_dir, recursive = TRUE, showWarnings = FALSE)

write_csv(
  harmonized_SQL_crosswalk,
  file.path(crosswalk_dir, "GAZP1_harmonized-SQL_crosswalk.csv")
)
...
DBI::dbWithTransaction(con, {
  ...
  append_if_rows(con, "location", stg_location)
  ...
  append_if_rows(con, "author_contributor", stg_author_contributor)
...
# Crosswalk table
upload_to_supabase(
  local_file = "crosswalk_tables/GAZP/GAZP1/GAZP1_harmonized-SQL_crosswalk.csv",
  destination_path = "GAZP/GAZP1/crosswalks/GAZP1_harmonized-SQL_crosswalk.csv",
...
upload_to_supabase(
  local_file =
    "data/harmonized/GAZP/GAZP1/GAZP1.xlsx",

  destination_path =
    "GAZP/GAZP1/harmonized/GAZP1.xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP1/description.docx",

  destination_path =
    "GAZP/GAZP1/source/description.docx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP1/Meta-data_BHPIrri.xlsx",

  destination_path =
    "GAZP/GAZP1/source/Meta-data_BHPIrri.xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP1/Meta-data_Pilbara_CEF Round 1.xlsx",

  destination_path =
    "GAZP/GAZP1/source/Meta-data_Pilbara_CEF_Round_1.xlsx",
...
upload_to_supabase(
  local_file =
    "data/source/GAZP/GAZP1/Pilbara CEF Round 1_example dataset.csv",

  destination_path =
    "GAZP/GAZP1/source/Pilbara_CEF_Round_1_example_dataset.csv",
