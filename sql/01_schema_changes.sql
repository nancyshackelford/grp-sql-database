...
-- Drop dependent views before altering source tables.
DROP VIEW IF EXISTS grp.full_paper;
DROP VIEW IF EXISTS grp.full_project;

-- Create project-level data accessibility table.
-- This is intentionally one-to-many from project to data accessibility records
-- to allow future flexibility for multiple datasets, access pathways, or tenancy-style structures.
CREATE TABLE grp.project_data_accessibility (
    data_accessibilityid integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    CONSTRAINT project_data_accessibility_projectid_fkey
...
-- Drop data-accessibility fields from paper.
-- Publication access fields remain in paper:
--   publication_doi
--   publication_url
ALTER TABLE grp.paper
    DROP COLUMN data_citation,
    DROP COLUMN creativecommons_license,
    DROP COLUMN use_conditions,
    DROP COLUMN date_received;

-- Drop project-level availability from project.
-- Availability now belongs in grp.project_data_accessibility.
